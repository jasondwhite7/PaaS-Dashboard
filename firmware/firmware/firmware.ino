...
void setup() {
  ...
  // Initialize O2 on Wire1
  Wire1.begin();
  if (oxygen.begin(OXYGEN_I2C_ADDR)) {
    Serial.println("O2 Sensor Initialized");
  } else {
    Serial.println("O2 Sensor Failed to Initialize");
  }
...
void loop() { 
  ...
  client.println(); serializeJson(data, client); 
